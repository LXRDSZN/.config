...
(function ($,storage, settings, CACHE, TABS,FILTERS, fnBuy) {
    var genres = [
    ...
    var activeTab = settings.activeTab, activeFilter = settings.activeFilter, payed = settings.payed, theme = settings.theme;
    //storage
    var liked = settings.liked, lastUsed = settings.lastUsed;
    ...
    function loadData(parent) {
        ...
        if (TABS.popular === activeTab){
            ...
            if (!payed){
                ...
                $cont.find('.enhancerbrowseunlockbtn').on('click',function(){
                    doBuy();
                    payed = true;
                    window.close();
                });
    ...
    function bindHead() {
        ...
        $sc.find('#enhancesearch').on('keyup',function () {
        ...
        if ($bind.length){
            ...
            $bind.on('click mouseup',function (e) {
                if (e.type !== 'mouseup' || e.which === 2){
                    ...
                    var $this = $(this);
                    if ($this.data('id')){
                        openinCurrentTab('https://www.netflix.com/browse/genre/'+$this.data('id'));
    ...
    function init() {
        ...
        theme && $('body').removeClass('SimklBrowseLight');
        ...
        $sc.find('.enhancerbrowsesearchblockmenu').on('click',function () {
            $sc.find('.enhancerbrowseblock').hide();
            $sc.find('.enhancerbrowsepagemenu').css('display', 'flex');

        });
        $sc.find('.enhancerbrowsepagemenuheader').on('click',function () {
            $sc.find('.enhancerbrowsepagemenu').hide();
            $sc.find('.enhancerbrowseblock').css('display', 'flex');
        });

        $sc.find(".enhancerbrowsesearchblockswitch").on('click',function () {
            theme = !$('body').toggleClass("SimklBrowseLight").hasClass('SimklBrowseLight');
            storage.setL(CACHE.theme, theme);
        });

        $sc.find('.enhancerbrowsepagemenucontentitem').on('click',function (e) {
            var $t = $(this);
            if ($t.hasClass('unlock')){
                $sc.find('.enhancerbrowsepagemenuheader').click();
                $sc.find('.enhancerbrowsemenusubtitle[data-v="'+TABS.full+'"]').click();
            }else if ($t.data('link')){
                var _link = $t.data('link');
                if (_link.substr(0,6) === 'mailto') window.location.href=_link;  else window.open(_link, '_blank');
            }
        });
        ...
        window.onunload = function(){
            ...
            settings.payed = payed;
            settings.liked = liked;
            settings.lastUsed = lastUsed;
    ...
    init();
})(jQuery,bg.d.storage, bg.d.settings, bg.d.CACHE, bg.d.TABS, bg.d.FILTERS, bg.d.doBuy);